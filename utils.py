...
def generate_cropped_images_and_keypoints_df(annot_df, src="./data/images/", dest="./data/cropped_images/", final_img_size=512):
  updated_data = {}
  ...
  for i in range(len(annot_df)):
    ...
    try:
      ...
      if width_bbox > height_bbox:
        ...
        cropped = result[ymin_bbox+yy-dir1:ymin_bbox+height_bbox+yy+dir1, xmin_bbox+xx:xmin_bbox+width_bbox+xx]
        cropped = cv2.resize(cropped, (512, 512))
        for keypt in keypoint_names:
          newX = int((512/width_bbox) * (int(annot_df[keypt+'_x'][i])-xmin_bbox))
          newY = int((512/width_bbox) * (int(annot_df[keypt+'_y'][i])-ymin_bbox+dir1))
        ...
        cropped = result[ymin_bbox+yy:ymin_bbox+height_bbox+yy, xmin_bbox+xx-dir1:xmin_bbox+width_bbox+xx+dir1]
        cropped = cv2.resize(cropped, (512, 512))
        for keypt in keypoint_names:
          newX = int((512/height_bbox) * (int(annot_df[keypt+'_x'][i])-xmin_bbox+dir1))
