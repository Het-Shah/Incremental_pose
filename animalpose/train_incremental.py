...
def main():
	...
	base_datasets_incremental_train = []
	...
	for animal_class in cfg.ANIMAL_CLASS_BASE:
		...
		n = int(cfg.TRAIN.BASE_DATA_FOR_INCREMENTAL * len(train_images_list))
		index = np.random.choice(train_images_list.shape[0], n, replace=False) 

		images_list_random = train_images_list[index]
		...
		base_dataset_incremental_temp = AnimalDatasetCombined(cfg.DATASET.IMAGES, cfg.DATASET.ANNOT, images_list_random, input_size=(512,512), output_size=(128,128), transforms = torchvision.transforms.Compose([ToTensor()]), train=True)

		train_datasets.append(train_tempset)
		val_datasets.append(val_tempset)
		base_datasets_incremental_train.append(base_dataset_incremental_temp)
	...
	base_incremental_train_dataset = torch.utils.data.ConcatDataset(base_datasets_incremental_train)
	...
	for i in range(int(len(cfg.ANIMAL_CLASS_INCREMENTAL)/cfg.INCREMENTAL_STEP)):
		...
		for animal_class in animal_classes:
			...
			n = int(cfg.TRAIN_INCREMENTAL.BASE_DATA_FOR_INCREMENTAL * len(train_images_list))
			index = np.random.choice(train_images_list.shape[0], n, replace=False) 
			...
			rolling_tempset = AnimalDatasetCombined(cfg.DATASET.IMAGES, cfg.DATASET.ANNOT, images_list_random, input_size=(512,512), output_size=(128,128), transforms = torchvision.transforms.Compose([ToTensor()]), train=True)

			curr_train_datasets.append(train_tempset)
			curr_val_datasets.append(val_tempset)
			curr_rolling_sets.append(rolling_tempset)

		if not cfg.TRAIN_INCREMENTAL.KD_LOSS: 
			for train_set in train_datasets_incremental:
				curr_train_datasets.append(train_set)

		if cfg.TRAIN_INCREMENTAL.KD_LOSS:
			kd_trainset = torch.utils.data.ConcatDataset(train_datasets_incremental)
			kd_train_loader = torch.utils.data.DataLoader(kd_trainset, batch_size=cfg.TRAIN_INCREMENTAL.BATCH_SIZE*num_gpu, shuffle=True)

		for rolling_set in curr_rolling_sets:
			train_datasets_incremental.append(rolling_set)
		...
		for ep in range(cfg.TRAIN_INCREMENTAL.BEGIN_EPOCH, cfg.TRAIN_INCREMENTAL.END_EPOCH):
			...
			if cfg.TRAIN_INCREMENTAL.KD_LOSS:
				...
				train_loss_kd, train_acc_kd = train_kd(opt, kd_train_loader, m, m_prev, criterion, optimizer, writer, phase="Incremental_Train"+str(i))
				logger.epochInfo('Incremental_Train'+str(i)+'_KD', opt.epoch, train_loss_kd, train_acc_kd)
			else:
				train_loss, train_acc = train(opt, incremental_train_loader, m, criterion, optimizer, writer, phase="Incremental_Train"+str(i))
