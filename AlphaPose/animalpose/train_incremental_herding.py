		...
		for animal_class in classes_till_now:
			...
			for fname in filename_list_classes[animal_class]:
				...
				for keypt in keypoint_names: 
					temp[1].append([annot_df.loc[annot_df['filename']==fname][keypt+'_x'].item(), 
							annot_df.loc[annot_df['filename']==fname][keypt+'_y'].item(), 
							annot_df.loc[annot_df['filename']==fname][keypt+'_visible'].item()])
			...
			if cfg.MEMORY_TYPE != 'fix': 
				samples_per_class = int(cfg.TRAIN_INCREMENTAL.BASE_DATA_FOR_INCREMENTAL*len(final_animal_vec))
			...
			if cfg.SAMPLING.STRATERGY == 'cluster':
				...
				for clus in range(cfg.SAMPLING.N_CLUSTERS):
					...
					# temp = keypoints_list[ClusterIndicesNumpy(clus, km.labels_)][:samples_per_cluster]
					for j in temp: 
